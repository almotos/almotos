...
function adicionarItem($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $existeNombre = $sql->existeItem('tipos_empleado', 'nombre', $datos['nombre']);
        ...
        if (empty($datos['nombre'])) {
            ...
            if ($idItem) {
                ...
                $objeto = new TipoEmpleado($idItem);
...
function modificarItem($id, $datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $existeNombre = $sql->existeItem('tipos_empleado', 'nombre', $datos['nombre'], 'id != "' . $id . '"');
...
function eliminarItem($id, $confirmado, $dialogo) {
    ...
    if (!$confirmado) {
        ...
        $arreglo1 = array('empleados', 'id_tipo_empleado = "'.$id.'"', $textos->id('EMPLEADOS'));//arreglo del que sale la info a consultar
...
function buscarItem($data, $cantidadRegistros = NULL) {
    ...
    if (empty($datos)) {
        ...
        if ($condicionales == '') {
            $condicion = '(te.nombre REGEXP "(' . implode('|', $palabras) . ')")';
        } else {
        ...
        $arregloItems = $objeto->listar($registroInicial, $registros, array('0'), $condicion, 'te.nombre');
...
function paginador($pagina, $orden = NULL, $nombreOrden = NULL, $consultaGlobal = NULL, $cantidadRegistros = NULL) {
    ...
    if (isset($consultaGlobal) && $consultaGlobal != '') {
        ...
        if ($data[1] != '') {
            ...
            $consultaGlobal = '(te.nombre REGEXP "(' . implode('|', $palabras) . ')")';
        }
    } else {
        $consultaGlobal = '';
    }
...
function listarItems($cadena) {
    global $sql;
    $respuesta = array();
    $consulta = $sql->seleccionar(array('tipos_empleado'), array('nombre'), '(nombre LIKE "%' . $cadena . '%") AND activo = "1" AND id != "0"', '', 'nombre ASC', 0, 20);
...
function eliminarVarios($confirmado, $cantidad, $cadenaItems) {
    ...
    $destino = '/ajax/tipos_empleado/eliminarVarios';
    $respuesta = array();
