...
function cosultarItem($id) {
    ...
    if (empty($id) || (!empty($id) && !$sql->existeItem('marcas', 'id', $id))) {
        $respuesta              = array();
...
function adicionarItem($datos = array()) {
    global $textos, $sql, $archivo_imagen, $modulo, $sesion_usuarioSesion;
    ...
    */
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::areaTexto('descripcion', 10, 40);
        $codigo .= HTML::parrafo($textos->id('IMAGEN'), 'negrilla margenSuperior');
        ...
        $existeNombre   = $sql->existeItem('marcas', 'nombre', $datos['nombre']);
...
function modificarItem($id, $datos = array()) {
   ...
   if (empty($id) || (!empty($id) && !$sql->existeItem('marcas', 'id', $id))) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::areaTexto('descripcion', 10, 40);
        $codigo .= HTML::parrafo($textos->id('IMAGEN'), 'negrilla margenSuperior');
        $codigo .= HTML::campoArchivo('imagen', 50, 255) . HTML::imagen($objeto->imagenMiniatura, 'imagenLinea', 'margenIzquierda');
        ...
        $existeNombre = $sql->existeItem('marcas', 'nombre', $datos['nombre'], 'id != "'.$id.'"');
        
        if (!empty($archivo_imagen['tmp_name'])) {
            $archivoInvalido = Recursos::validarArchivo($archivo_imagen, array('jpg', 'png', 'gif', 'jpeg'));
        ...
        } elseif ($archivoInvalido) {
            $respuesta['mensaje'] = $textos->id('ERROR_FORMATO_IMAGEN');

        } else {
            
            $ventanaDialogo = $datos['dialogo'];
            unset($datos['dialogo']);
...
function eliminarItem($id, $confirmado, $dialogo) {
    ...
    */
    ...
    if (!$confirmado) {
        ...
        $arreglo1           = array('articulos', 'id_marca = "'.$id.'"', $textos->id('ARTICULOS'));//arreglo del que sale la info a consultar
        $arreglo2           = array('motos',     'id_marca = "'.$id.'"', $textos->id('MOTOS'));
...
function buscarItem($data, $cantidadRegistros = NULL) {
    ...
    if (empty($datos)) {
        ...
        $arregloItems = $objeto->listar($registroInicial, $registros, array('0'), $condicion, 'm.nombre');
...
function listarItems($cadena) {
    ...
    $consulta = $sql->seleccionar(array('marcas'), array('id', 'nombre'), '(nombre LIKE "%' . $cadena . '%") AND activo = "1"', '', 'nombre ASC', 0, 20);
...
function eliminarVarios($confirmado, $cantidad, $cadenaItems) {
    ...
    */
    ...
    $destino    = '/ajax/marcas/eliminarVarios';
    $respuesta  = array();
