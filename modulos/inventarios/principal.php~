...
$objeto         = new Inventario(); /* creacion del objeto */
...
//    $botonAdicionar = HTML::contenedor(HTML::botonCargarMasivo($objeto->urlBase, $textos->id('ADICIONAR_MASIVO')), 'flotanteDerecha margenInferior', 'botonCargarMasivo' . ucwords(strtolower($modulo->nombre)) . '');
...
if ((isset($sesion_usuarioSesion) && $puedeImprimirBarcodePdf) || isset($sesion_usuarioSesion) && $sesion_usuarioSesion->idTipo == 0) {
    $botonImprimirBarcodePdf = HTML::contenedor(HTML::boton('libreta', $textos->id('IMPRIMIR_VARIOS_BARCODES'), 'directo', '', 'botonImprimirVariosBarcode', '', array('ruta' => '/ajax/' . $modulo->url . '/imprimirVariosBarcode')), 'flotanteDerecha margenInferior botonImprimirVariosBarcode oculto', 'botonImprimirVariosBarcode' . ucwords(strtolower($modulo->nombre)) . '');
}
...
$botonesSuperiores          = HTML::contenedor($buscador . $botonAdicionar . $botonCargarMasivo . $botonEliminar . $botonImprimirBarcodePdf . $chkMarcarFilas . $campoNumRegistros . $contenedorNotificaciones, '', 'botonesSuperioresModulo');
