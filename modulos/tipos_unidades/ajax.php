 ...
 * */
global $url_accion, $forma_procesar, $forma_id, $forma_datos, $forma_pagina, $forma_cantidadRegistros, $forma_cantidad, $forma_cadenaItems, $forma_orden, $forma_nombreOrden, $url_cadena, $forma_dialogo, $forma_consultaGlobal;
...
function adicionarItem($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $existeNombre = $sql->existeItem('tipos_unidades', 'nombre', $datos['nombre']);
...
function modificarItem($id, $datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $existeNombre = $sql->existeItem('tipos_unidades', 'nombre', $datos['nombre'], 'id != "' . $id . '"');
        ...
        if (empty($datos['nombre'])) {
            ...
            if ($idItem) {
                ...
                $celdas = array($objeto->codigo, $objeto->nombre, $estado);
...
function eliminarItem($id, $confirmado, $dialogo) {
    ...
    if (!$confirmado) {
        ...
        $arreglo1 = array('unidades', 'id_tipo_unidad = "'.$id.'"', $textos->id('UNIDADES'));//arreglo del que sale la info a consultar
...
function buscarItem($data, $cantidadRegistros = NULL) {
    ...
    if (empty($datos)) {
        ...
        if ($condicionales == '') {
            $condicion = '(tu.nombre REGEXP "(' . implode('|', $palabras) . ')")';
        } else {
        ...
        $arregloItems = $objeto->listar($registroInicial, $registros, array('0'), $condicion, 'tu.nombre');
...
function paginador($pagina, $orden = NULL, $nombreOrden = NULL, $consultaGlobal = NULL, $cantidadRegistros = NULL) {
    ...
    if (isset($consultaGlobal) && $consultaGlobal != '') {
        ...
        if ($data[1] != '') {
            ...
            $consultaGlobal = '(tu.nombre REGEXP "(' . implode('|', $palabras) . ')")';
        }
    } else {
        $consultaGlobal = '';
    }
...
function listarItems($cadena) {
    global $sql;
    $respuesta = array();
    $consulta = $sql->seleccionar(array('tipos_unidades'), array('nombre'), '(nombre LIKE "%' . $cadena . '%") AND activo = "1"', '', 'nombre ASC', 0, 20);
...
function eliminarVarios($confirmado, $cantidad, $cadenaItems) {
    ...
    $destino = '/ajax/tipos_unidades/eliminarVarios';
    $respuesta = array();
