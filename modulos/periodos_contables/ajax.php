...
function cosultarItem($id) {
    ...
    if (empty($id) || (!empty($id) && !$sql->existeItem('periodos_contables', 'id', $id))) {
        $respuesta              = array();
...
/**
 * Funcion con doble comportamiento. La primera llamada (con el arreglo de datos vacio)
 * muestra el formulario para el ingreso del registro. El destino de este formulario es esta 
 * misma funcion, pero una vez viene desde el formulario con el arreglo datos cargado de valores
 * se encarga de validar la informacion y llamar al metodo adicionar del objeto.
 * 
...
function adicionarItem($datos = array()) {
    ...
    */
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::parrafo($textos->id('FECHA_INICIAL'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[fecha_inicial]', 20, 20, '', 'campoObligatorio fechaAntigua campoCalendario');
        $codigo .= HTML::parrafo($textos->id('FECHA_FINAL'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[fecha_final]', 20, 20, '', 'campoObligatorio fechaAntigua campoCalendario');  
        ...
        $respuesta['error'] = true;
        $sql->depurar = true;
        $existeNombre = $sql->existeItem('periodos_contables', 'nombre', $datos['nombre']);
        $existeCodigo = $sql->existeItem('periodos_contables', 'codigo', $datos['codigo']);
        ...
        elseif (empty($datos['nombre'])) {
            ...
            if ($idItem) {
                ...
                $objeto = new PeriodoContable($idItem);

                $estado = ($objeto->activo) ? HTML::frase($textos->id('ACTIVO'), 'activo') : HTML::frase($textos->id('INACTIVO'), 'inactivo');

                $celdas    = array($objeto->codigo,$objeto->nombre, $objeto->fechaInicial, $objeto->fechaFinal, $estado);
                $claseFila = '';
                $idFila    = $idItem;
 ...
 * misma funcion, pero una vez viene desde el formulario con el arreglo datos cargado de valores
 * se encarga de validar la informacion y llamar al metodo modificar del objeto.
 * 
 * @global recurso $textos  = objeto global de gestion de los textos de idioma
 * @global recurso $sql     = objeto global de interaccion con la BD
...
function modificarItem($id, $datos = array()) {
    ...
    */
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::parrafo($textos->id('FECHA_INICIAL'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[fecha_inicial]', 20, 20, $objeto->fechaInicial, 'campoObligatorio fechaAntigua campoCalendario');
        $codigo .= HTML::parrafo($textos->id('FECHA_FINAL'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[fecha_final]', 20, 20, $objeto->fechaFinal, 'campoObligatorio fechaAntigua campoCalendario');  
        ...
        $existeCodigo = $sql->existeItem('periodos_contables', 'codigo', $datos['codigo'], 'id != "' . $id . '"');
        $existeNombre = $sql->existeItem('periodos_contables', 'nombre', $datos['nombre'], 'id != "' . $id . '"');
...
/**
 * Funcion con doble comportamiento. La primera llamada (con el parametro $confirmado vacio)
 * muestra el formulario de confirmacion de eliminacion del registro. El destino de este formulario es esta 
 * misma funcion, pero una vez viene desde el formulario con el parametro $confirmado en "true"
 * se encarga de validar la informacion y llamar al metodo modificar del objeto.
 * 
 * @global recurso $textos  = objeto global de gestion de los textos de idioma
 * @global recurso $sql     = objeto global de interaccion con la BD
...
function buscarItem($data, $cantidadRegistros = NULL) {
    ...
    if (empty($datos)) {
        ...
        if ($condicionales == '') {
            $condicion = '(pc.nombre REGEXP "(' . implode('|', $palabras) . ')")';
        ...
        $sql->depurar = true;
        $arregloItems = $objeto->listar($registroInicial, $registros, array('0'), $condicion, 'pc.nombre');
...
function paginador($pagina, $orden = NULL, $nombreOrden = NULL, $consultaGlobal = NULL, $cantidadRegistros = NULL) {
    ...
    if (isset($consultaGlobal) && $consultaGlobal != '') {
        ...
        if ($data[1] != '') {
            ...
            $consultaGlobal = '(pc.nombre REGEXP "(' . implode('|', $palabras) . ')")';
...
function listarItems($cadena) {
    ...
    $respuesta = array();
    $consulta  = $sql->seleccionar(array('periodos_contables'), array('id', 'nombre'), '(nombre LIKE "%' . $cadena . '%" || codigo LIKE "%' . $cadena . '%") AND activo = "1"', '', 'nombre ASC', 0, 20);
 ...
 * boton que aparece llamado "Eliminar varios"
 * 
...
function eliminarVarios($confirmado, $cantidad, $cadenaItems) {
    ...
    * Verificar si el usuario que esta en la sesion tiene permisos para esta accion
    */
    $puedeEliminarMasivo = Perfil::verificarPermisosBoton('botonEliminarMasivoPeriodoContable', $modulo->nombre);
    ...
    $destino   = '/ajax/periodos_contables/eliminarVarios';
    $respuesta = array();
    ...
    if (!$confirmado) {
        ...
        foreach ($arregloIds as $val) {
            $objeto = new PeriodoContable($val);
