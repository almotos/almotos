...
function adicionarItem($datos) {
    ...
    if ($continuar) {
        $datosFactura = array(
            ...
            'datos[fecha_vto_factura]'          => $datos['fecha_vto_factura'],
            'datos[modo_pago]'                  => $datos['modo_pago'],
            'datos[iva]'                        => $datos['iva'],
    //        'datos[retecree]'                   => $datos['retecree'],
            ...
            'datos[id_factura_temporal]'        => $datos['id_factura_temporal'],
        ...
        if (empty($datos['es_orden_compra'])) {
            ...
            /* $listaTiposCompra = array();

            $consulta = $sql->seleccionar(array('tipos_compra'), array('id', 'nombre'), ' id != 0', '', 'nombre ASC');

            if ($sql->filasDevueltas) {
                while ($dato = $sql->filaEnObjeto($consulta)) {
                    $listaTiposCompra[$dato->id] = $dato->nombre;

                }

            }    

            $botones .= HTML::frase($textos->id('MODO_PAGO'), 'margenSuperior');
            $botones .= HTML::listaDesplegable('datos[modo_pago]', $listaTiposCompra, '', 'margenSuperior selectChosen', 'listaMedioPago', 'Seleccionar...', array('ayuda' => $textos->id('SELECCIONE_MEDIO_DE_PAGO')));
            */
            
            /* Inicio manejo lógica de campos retenciones */            
            
            $contabilidad = new Contabilidad();
            $respuestaRetenciones  = $contabilidad->generarCamposRetenciones($datos['id_proveedor'], $datos['total']);   
            ...
            $campos .= HTML::campoTexto('datos[efectivo]', 20, 255, $respuestaRetenciones["total_a_pagar"], 'margenIzquierda campoDinero', 'campoEfectivo');  
            ...
            $campos .= HTML::campoTexto('datos[tarjeta]', 20, 255, '', 'margenIzquierda campoDinero', 'campoTarjeta', array('disabled' => 'disabled'));
            $campos .= HTML::parrafo('', 'negrilla margenSuperior');
            $campos .= HTML::campoChequeo('', false, '', 'checkCheque');
            $campos .= HTML::frase($textos->id('CHEQUE').' .......', 'negrilla margenSuperior margenIzquierda');
            $campos .= HTML::campoTexto('datos[cheque]', 20, 255, '', 'margenIzquierda campoDinero', 'campoCheque', array('disabled' => 'disabled'));
            $campos .= HTML::parrafo('', 'negrilla margenSuperior');
            $campos .= HTML::campoChequeo('', false, '', 'checkCredito');
            $campos .= HTML::frase($textos->id('CREDITO').' ........', 'negrilla margenSuperior margenIzquierda');
            $campos .= HTML::campoTexto('datos[credito]', 20, 255, '', 'margenIzquierda campoDinero', 'campoCredito', array('disabled' => 'disabled'));            
...
function imprimirFacturaCompraPdf($datos) {
    ...
    $pdf->SetFont('times', '', 7);
    $pdf->Cell(20, 7, $objeto->proveedor, 0, 0, 'L');
...
//    
//    $pdf->SetFont('times', 'B', 7);
//    $pdf->Cell(170, 7, $textos->id("RETECREE") . ': ', 0, 0, 'R');
//    $pdf->SetFont('times', '', 7);
//    $pdf->Cell(30, 7, '$'.Recursos::formatearNumero($objeto->retecree, '$'), 0, 0, 'R');    
...
function imprimirFacturaCompraPos($datos) {
    ...
    $tirilla_proveedor  = $textos->id("PROVEEDOR") . "  : " . $objeto->proveedor . "\n";
    $tirilla_proveedor .= $textos->id("NIT_PROVEEDOR") . " : " . $objeto->idProveedor . "\n";
    ...
    $pieTirilla .= $textos->id("IVA") . ": $" . $objeto->iva . "\n\n";     
