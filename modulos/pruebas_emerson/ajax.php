...
function cosultarItem($id) {
    ...
    $codigo .= HTML::parrafo($textos->id('APELLIDO'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->apellido, '', '');

    $codigo .= HTML::parrafo($textos->id('DIRECCION'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->direccion, '', '');
...
function adicionarItem($datos = array()) {
	...
	$codigo .= HTML::parrafo($textos->id('APELLIDO'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[apellido]', 40, 255);
        $codigo .= HTML::parrafo($textos->id('DIRECCION'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[direccion]', 40, 255);
	$codigo .= HTML::parrafo($textos->id('CELULAR'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[celular]', 40, 255);
	...
	$existeApellido = $sql->existeItem('impuestos', 'apellido', $datos['apellido']);
        ...
        } elseif ($existeApellido) {
            $respuesta['mensaje'] = $textos->id('ERROR_EXISTE_APELLIDO');
        } else {
            $idItem = $objeto->adicionar($datos);
            if ($idItem) {
                /*                 * ************** Creo el nuevo item que se insertara via ajax *************** */
                $objeto = new PruebaEmerson($idItem);
                ...
                $celdas = array($objeto->nombre,$objeto->apellido,$objeto->direccion,$objeto->celular,$aplicaProveedores , $aplicaClientes, $estado);
...
function modificarItem($id, $datos = array()) {
	...
	$codigo .= HTML::parrafo($textos->id('APELLIDO'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[apellido]', 40, 255, $objeto->apellido);
        $codigo .= HTML::parrafo($textos->id('DIRECCION'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[direccion]', 40, 255, $objeto->direccion);
	$codigo .= HTML::parrafo($textos->id('CELULAR'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[celular]', 40, 255, $objeto->celular);
