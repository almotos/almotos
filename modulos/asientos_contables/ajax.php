...
function cosultarItem($id) {
    ...
    if (empty($id) || (!empty($id) && !$sql->existeItem('asientos_contables', 'id', $id))) {
        $respuesta              = array();
    ...
    $objeto         = new AsientoContable($id);
    $respuesta      = array();

    $codigo  = HTML::campoOculto('procesar', 'true');
    $codigo .= HTML::campoOculto('id', $id);
    $codigo .= HTML::parrafo($textos->id('CUENTA'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->cuenta, '', '');
    $codigo .= HTML::parrafo($textos->id('COMPROBANTE'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->comprobante, '', '');
    $codigo .= HTML::parrafo($textos->id('NUMERO_COMPROBANTE'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->numeroComprobante, '', '');
    $codigo .= HTML::parrafo($textos->id('FECHA'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->fecha, '', '');
    $codigo .= HTML::parrafo($textos->id('CONCEPTO'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->concepto, '', '');
    $codigo .= HTML::parrafo($textos->id('CREDITO'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->credito, '', '');
    $codigo .= HTML::parrafo($textos->id('DEBITO'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($objeto->debito, '', '');
...
function adicionarItem($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::parrafo($textos->id('CUENTA'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[cuenta]', 40, 255, '', 'autocompletable campoObligatorio', '', array('title' => HTML::urlInterna('SEDES_EMPRESA', 0, true, 'listar')), $textos->id('AYUDA_USO_AUTOCOMPLETAR'), HTML::urlInterna('SEDES_EMPRESA', 0, true, 'add'), 'datos[id_sede]');
