...
$objeto = new Evento(); /* creacion del objeto */
 ...
 */
$puedeAdicionarMasivo = Perfil::verificarPermisosBoton('botonAdicionarMasivo', $modulo->id);
$botonAdicionarMasivo = '';
if ((isset($sesion_usuarioSesion) && $puedeAdicionarMasivo) || isset($sesion_usuarioSesion) && $sesion_usuarioSesion->idTipo == 0) {
    $botonAdicionarMasivo = HTML::contenedor(HTML::botonAdicionarMasivo($objeto->urlBase, $textos->id('ADICIONAR_MASIVO')), 'flotanteDerecha margenInferior', 'botonAdicionarMasivo' . ucwords(strtolower($modulo->nombre)) . '');
}
...
$botonesSuperiores = HTML::contenedor($buscador . $botonAdicionar . $botonAdicionarMasivo . $botonEliminar . $chkMarcarFilas . $contenedorNotificaciones, '', 'botonesSuperioresModulo');
