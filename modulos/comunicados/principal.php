...
if (isset($url_ruta)) {
    ...
    if (isset($blog->id)) {
        ...
        $comentarios = $comentario->contar("BLOGS", $blog->id);
            ...
            $meGusta = Recursos::cargarMegusta($blog->idModulo, $blog->id, $sesion_usuarioSesion->idTipo);

            }else{
            $meGusta = Recursos::mostrarMegusta($blog->idModulo, $blog->id);
...
/**********************Calcular el total de registros activos***************************/     
         ...
         foreach ($blog->listar2(0, 0, $excluidas, "", $idTipo, $blog->idModulo) as $elemento) {
...
/**************************************************************************************/
        ...
        $reg = sizeof($blog->listar2(0, 0, $excluidas, "", $idTipo, $blog->idModulo));



    foreach ($blog->listar2($registroInicial, $registros, $excluidas, "", $idTipo, $blog->idModulo) as $elemento) {
        $fila++;
        $item   = "";
        $celdas = array();
