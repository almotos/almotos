...
function adicionarItem($datos = array()) {
    ...
    */
    ...
    if (empty($datos)) {
        //creo una lista desplegable para el tipo de compra
...
//        $listaDesplegableCategoria = HTML::listaDesplegable('datos[categoria]', array('1' => $textos->id('CATEGORIA_1'), '2' => $textos->id('CATEGORIA_2'), '3' => $textos->id('CATEGORIA_3')), '', '', 'listaTipoGrupo', '', '', $textos->id('AYUDA_SELECCIONAR_CATEGORIA'));
        ...
        $existeNombre = $sql->existeItem('tipos_compra', 'nombre', $datos['nombre']);
...
//        $codigo .= $listaDesplegableCategoria;           
        ...
        $existeNombre = $sql->existeItem('tipos_compra', 'nombre', $datos['nombre'], 'id != "' . $id . '" ');
...
function listarItems($cadena) {
    global $sql;
    $respuesta = array();
    $consulta = $sql->seleccionar(array('tipos_compra'), array('id', 'nombre'), 'nombre LIKE "%' . $cadena . '%" AND activo = "1" ', '', 'nombre ASC', 0, 20);
...
function eliminarVarios($confirmado, $cantidad, $cadenaItems) {
    ...
    */
    ...
    $destino   = '/ajax/tipos_compra/eliminarVarios';
    $respuesta = array();
...
/**
 * Función que carga el formulario para adicionar cuentas contables a un tipo de compra
 * 
...
function adicionarCuenta($id, $datos = array()) {
    ...
    */
    ...
    $destino = '/ajax/tipos_compra/adicionarCuenta';
    $respuesta = array();

    if (empty($datos)) {
        
    
        if (empty($id) || (!$sql->existeItem('tipos_compra', 'id', $id))) {
            $respuesta              = array();
            $respuesta['error']     = true;
        ...
        $codigo = HTML::campoOculto('procesar', 'true');
        $codigo .= HTML::campoOculto('datos[dialogo]', '', 'idDialogo');
        $codigo .= HTML::campoOculto('datos[id_tipo_compra]', $id, ''); //id del tipo de compra
...
function eliminarcuenta($id, $confirmado, $dialogo) {
    ...
    $destino    = '/ajax/tipos_compra/eliminarCuenta';
    $respuesta  = array();
