...
function cosultarItem($id) {
    ...
    $codigo  = HTML::parrafo($textos->id('ID_ARTICULO') . ': ' . HTML::frase($objeto->id, 'sinNegrilla'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($textos->id('ARTICULO') . ': ' . HTML::frase($objeto->articulo, 'sinNegrilla'), 'negrilla margenSuperior');
    $codigo .= HTML::parrafo($textos->id('BODEGA') . ': ' . HTML::frase($objeto->bodega , 'sinNegrilla'), 'negrilla margenSuperior');
...
function cosultarMasDelItem($id, $destino, $pestana = '') {
    ...
    if($pestana == 'kardex'){
        ...
        $selectorSedes = HTML::listaDesplegable('datos[sede]', $listaSedes, $sesion_usuarioSesion->sede->id, $clase, 'selectorSedes', '', array("onchange" => "seleccionarBodegas($(this))"), '');    
        ...
        $pestana4  = HTML::parrafo( HTML::frase($textos->id('ULTIMO_PRECIO_COMPRA'), 'negrilla subtitulo').HTML::frase( ($objeto->ultimoPrecioCompra != '') ? '$ '.$objeto->ultimoPrecioCompra : '$ 0', 'subtitulo margenIzquierdaDoble'), 'margenSuperior');
        $pestana4 .= HTML::parrafo( HTML::frase( str_replace('%1', $diasPromedio, $textos->id('PRECIO_PROMEDIO_COMPRA')), 'negrilla subtitulo').HTML::frase(($objeto->precioPromedioCompra != '0') ? '$ '.$objeto->precioPromedioCompra : '$ 0', 'subtitulo margenIzquierdaDoble') , 'margenSuperior');
        //$pestana4 .= HTML::campoOculto('data-barra-kardex-articulo', $objeto->codigoGraficoBarras(), 'data-barra-kardex-articulo');
        $pestana4 .= HTML::canvas('', 'graficoBarras', 'graficoKardexArticulo', array('width' => '400px', 'height' => '300px'));
...
function adicionarItem($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::campoTexto('datos[articulo]', 40, 255, '', 'autocompletable campoObligatorio', '', array('title' => HTML::urlInterna('ARTICULOS', 0, true, 'listar')), $textos->id('AYUDA_USO_AUTOCOMPLETAR'), HTML::urlInterna('ARTICULOS', 0, true, 'add'), 'datos[id_articulo]');
        $codigo .= HTML::parrafo($textos->id('BODEGA'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[bodega]', 40, 255, '', 'autocompletable campoObligatorio', '', array('title' => HTML::urlInterna('BODEGAS', 0, true, 'listar')), $textos->id('AYUDA_USO_AUTOCOMPLETAR'), HTML::urlInterna('BODEGAS', 0, true, 'add'), 'datos[id_bodega]');
    ...
    if (empty($datos)) {
        ...
        if (empty($datos['subgrupo'])) {
            ...
            if ($idItem) {
                ...
                $celdas = array((int) $objeto->$idPrincipalArticulo, $objeto->nombre, $objeto->linea, $objeto->subgrupo, $objeto->codigoPais, '$ '.$objeto->precio1, '$ '.$objeto->ultimoPrecioCompra, $objeto->completo);
...
function listarArticulosVenta($cadena, $bodega) {
    ...
    if (ctype_digit($cadena)) { //verifico si la cadena que llega es un numero    
        $consulta = $sql->seleccionar(array('articulos'), array('id', 'nombre', 'precio1', $idPrincipalArticulo), '('.$idPrincipalArticulo.' LIKE "%' . $cadena . '%" OR referencia LIKE "%' . $cadena . '%") ', '', 'nombre ASC', 0, 15);
    
    } else { //si no es un numero, busco por nombre y referencia
        $consulta = $sql->seleccionar(array('articulos'), array('id', 'nombre', 'precio1', $idPrincipalArticulo), '(nombre LIKE "%' . $cadena . '%" OR referencia LIKE "%' . $cadena . '%")  ', '', 'nombre ASC', 0, 15);
...
function listarArticulosCompra($cadena, $bodega) {
    ...
    //en un rediseño se debe convertir en una sola función, detectar quien la solicita y asi hacer la consulta
    $respuesta  = array();
    ...
    if (ctype_digit($cadena)) {
        $consulta = $sql->seleccionar($tablas, $columnas, '('.$idPrincipalArticulo.' LIKE "%' . $cadena . '%" OR referencia LIKE "%' . $cadena . '%")  ', '', 'nombre ASC', 0, 50);
        
    } else {
        $consulta = $sql->seleccionar($tablas, $columnas, '(replace(nombre, " ", "") LIKE "%' . $cadena . '%" OR referencia LIKE "%' . $cadena . '%")  ', '', 'nombre ASC', 0, 50);
...
function cosultarTodos() {
    ...
    $item .= $objeto->generarTabla($arregloItems, $datosPaginacion, true);
...
function moverMercancia($id) {
    ...
    $codigo .= HTML::parrafo(HTML::frase($textos->id("ARTICULO"), 'subtitulo') . HTML::frase($textos->id('BODEGA_ORIGEN'), 'flotanteDerecha margenDerecha150 negrilla subtitulo'), "negrilla margenSuperior");
    
    if (strlen($objeto->nombre) > 60) {
        $objeto->nombre = HTML::frase(substr($objeto->nombre, 0, 60) . '...', '', '', array('ayuda' => $objeto->nombre));
    }
    $codigo .= HTML::parrafo(HTML::frase($objeto->nombre, 'ancho50por100') . HTML::frase($textos->id('SELECCIONA_ORIGEN'), 'flotanteDerecha margenDerecha100', 'bodegaOrigen'), "");

    $codigo .= HTML::parrafo(HTML::frase($textos->id("PRESENTACION"), 'subtitulo') . HTML::frase($imagen1 . $textos->id('BODEGA_DESTINO'), 'flotanteDerecha margenDerecha150 negrilla subtitulo'), "negrilla margenSuperiorTriple");
    ...
    $selectorSedes  = HTML::listaDesplegable('datos[sede]', $listaSedes, '', 'margenDerecha', 'selectorSedes', $textos->id('SELECCIONAR') . '...', array(), '');
    ...
    $codigo .= HTML::parrafo($objeto->unidad, "");
    $codigo .= HTML::parrafo($listaBodegas, 'flotanteDerecha');


    $codigo .= HTML::parrafo($imagen2 . HTML::frase($textos->id("BODEGA"), 'subtitulo') . HTML::frase($textos->id('CANTIDAD'), 'flotanteDerecha margenDerecha300 negrilla subtitulo'), "negrilla margenSuperior50");
    ...
    $codigo .= HTML::contenedor($codigo1, 'contenedorListaBodegasMovimiento', 'contenedorListaBodegasMovimiento');

    $codigo .= HTML::parrafo($textos->id('CANTIDAD_A_MOVER'), 'negrilla margenSuperior50', 'textoCantidadAMover');
    $codigo .= $imagen3 . HTML::campoTexto('datos[cantidad]', 10, 10, '', 'margenSuperior margenIzquierda rangoNumeros soloNumeros', 'campoCantidadAMover', array('rango' => ''));

    $codigo .= HTML::parrafo($imagen5 . HTML::boton("cancelar", $textos->id("RESTAURAR"), " margenIzquierda directo", "botonOk", "botonRestaurarValores"), "margenSuperiorDoble");
    $codigo .= HTML::parrafo($imagen4 . HTML::boton("chequeo", $textos->id("MOVER_MERCANCIA"), "directo", "", "botonAceptarMovimiento"), "margenSuperiorDoble flotanteDerecha margenDerechaDoble");
...
function adicionarMasivo($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $nombre .= HTML::campoArchivo('masivo', 50, 255, 'masivo');
        ...
        $filas = array(
            array(
                HTML::parrafo($textos->id('ARTICULO'), 'negrilla margenSuperior'),
                HTML::listaDesplegable('datos[id_articulo]', array('' => ''), '', 'selectorCampo', 'plu_interno', '', array('onChange' => 'seleccionarCampo(this)'))
            ),
            array(
                HTML::parrafo($textos->id('CANTIDAD'), 'negrilla margenSuperior'),
                HTML::listaDesplegable('datos[cantidad]', array('' => ''), '', 'selectorCampo', 'nombre', '', array('onChange' => 'seleccionarCampo(this)'))
            ),
            array(
                HTML::parrafo($textos->id('BODEGA'), 'negrilla margenSuperior'),
                HTML::listaDesplegable('datos[id_bodega]', array('' => ''), '', 'selectorCampo', 'codigo_oem', '', array('onChange' => 'seleccionarCampo(this)'))
            ),
        );
...
//    if($tipo == "barras"){
//        $arregloGrafico     =  $objeto->datosGraficoBarras($fechaInicio, $fechaFin);
//        $arregloLeyendas    =  $objeto->leyendasGraficoBarras($fechaInicio, $fechaFin);
//    }
    
    $arregloLeyendas = 
    
    $arregloGrafico  = array(
                            'legends'   => array("Enero","February","March","April","May","June","July"),
                            'datasets'  => array(
                                                    array(
    ...
    $respuesta['datos']  = $arregloGrafico;
