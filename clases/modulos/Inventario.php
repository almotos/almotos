...
class Inventario
{
    ...
    public function __construct($id = NULL)
    {
        ...
        $this->registros = $sql->obtenerValor('inventarios', 'COUNT(id)', 'id != "0"');
        //Saber el numero de registros activos
        $this->registrosActivos = $sql->obtenerValor('inventarios', 'COUNT(id)', 'id != "0"');     
        ...
        if ($existeArticulo) {//if exists
            ...
            if ($existeInventario) {//if exists
                ...
                if ($sesion_configuracionGlobal->facturarNegativo) {//se verifica que el sistema permita facturar en negativo
                    ...
                    $response = false; //si no permite facturar en negativo informaria al usuario con la respuesta de la funcion
    ...
    * 
    * todo adicionar manejo de transacciones
     *
     */
...
//        $botonesExtras      = array($moverMercancia);
        ...
        $botonDerecho       = HTML::crearMenuBotonDerecho('INVENTARIOS', array(), array('borrar' => true));
