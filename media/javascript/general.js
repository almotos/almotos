...
/* 
 * Archivo Varios Compuesto por:
 * 
 * Jquery v 1.7.2
 * Jquery Cookie
 * Jquery Form Plugin v 3.09
 * Jquery tooltip v1.2
 * Jquery validation plugin v1.6
...
;
(function($){
    var helper={},current,alt,tID,IE=$.browser.msie&&/MSIE\s(5\.5|6\.)/.test(navigator.userAgent),track=false;
    $.tooltip={
        blocked:false,
        defaults:{
            delay:350,
...
$.fn.extend({
    tooltip:function(settings){
        ...
        return this.each(function(){
            ...
            this.tooltipText=$(this).attr("ayuda");
//            $(this).removeAttr("ayuda");
//            this.alt="";
...
function save(){
    ...
    if(settings(this).bodyHandler){
        ...
        var parts=alt.split(settings(this).showBody);
        helper.alt.html(parts.shift()).show();
        helper.body.empty();
...
/**
 *
 *Plugin Jquery para utilizar el evento onShow en el caso de que un elemento
 *oculto se muestre en el DOM
 *
 **/
(function($){
  $.fn.extend({ 
          ...
          if($(this).is(':hidden')){
            var checkVis = function(){
              if($(obj).is(':visible')){
                callback.call();
                if(bindopt){
                  $('body').unbind('click keyup keydown mousemove', checkVis);
                }
              }                         
            }
            $('body').bind('click keyup keydown mousemove', checkVis);
          }
          else{
            callback.call();
          }
